    ...
    publicService.service("register", ["$rootScope", "$compile", '$state',function ($rootScope, $compile, $state) {
        ...
        this.addToTabs = function (obj, data) {
            ...
            if (aNewObj.type === 'multiple') {
                ...
                self.tabindex++;
                this.openChildTab(aNewObj, data);
            } else {
                if (flag) {
        ...
        this.checkTabsScale = function () {
            if (Number(window.getComputedStyle(document.getElementById("tabScrollor")).width.slice(0, -2)) < ($rootScope.tabs ? 0 : ($rootScope.tabs.length * 128)) ) {
                ...
                angular.element(document.getElementById("tabScrollor"))[0].scrollLeft = (index+1) * 128 +80 - Number(window.getComputedStyle(document.getElementById("tabScrollor")).width.slice(0, -2));
